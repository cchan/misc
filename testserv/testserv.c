...
int main(int argc, char *argv[])
{
  ...
  while(1){
    ...
    if ((newsockfd = accept4(sockfd, (struct sockaddr *) &cli_addr, &clilen, SOCK_NONBLOCK | SOCK_CLOEXEC)) < 0)
      continue; //error("on accept");

    //int yes = 1;
    //if (setsockopt(newsockfd, IPPROTO_TCP, TCP_NODELAY, &yes, sizeof(yes)) == -1) {
    //  close(newsockfd);
    //  continue;
    ...
    shutdown(newsockfd, SHUT_RDWR); // sleep(1); // http://blog.csdn.net/CPP_CHEN/article/details/29864509
    close(newsockfd);
  }
  close(sockfd);
  return 0;
